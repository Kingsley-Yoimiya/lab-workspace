...
ARCHIVE_ROOT="/afs-a3-241ceshi-shared/montyyin/archive/${ARCHIVE_STAMP}"
...
pack /afs-a3-241ceshi-shared/montyyin/results/dense_failslow/20260713_001230 \
     \"\$ARCH/dense_failslow_20260713_001230.tgz\"
pack /afs-a3-241ceshi-shared/montyyin/results/dense_failslow_gbsprop/20260713_071316 \
     \"\$ARCH/dense_failslow_gbsprop_20260713_071316.tgz\"
pack /afs-a3-241ceshi-shared/montyyin/results/mfu_moe_scale/20260712_181247 \
     \"\$ARCH/mfu_moe_scale_181247.tgz\"
pack /afs-a3-241ceshi-shared/montyyin/results/mfu_moe_scale/20260712_221912 \
...
if [[ -f "$REMOTE_DIR/parse_failslow_gap.py" ]]; then
  for f in parse_failslow_gap.py parse_train_mfu_log.py remote_finalize_reports.py; do
    ...
    base64 -w0 "$REMOTE_DIR/$f" | vcctl pod exec -i "$POD" -- bash -lc \
      "base64 -d | tee /afs-a3-241ceshi-shared/montyyin/lab-workspace/scripts/cluster/$f >/dev/null && wc -c /afs-a3-241ceshi-shared/montyyin/lab-workspace/scripts/cluster/$f"
  done
fi
...
export RUN_ROOT="/afs-a3-241ceshi-shared/montyyin/results/moe_failslow/${STAMP_MOE}"
