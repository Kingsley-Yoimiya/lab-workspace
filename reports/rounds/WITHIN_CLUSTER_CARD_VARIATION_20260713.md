...
体质总览箱线（一图看多指标齐性，箱体窄=齐）：

![昇腾 box overview](card_constitution_20260711_figs/box_overview.svg)

HBM 相对集群中位偏差热图（host×device，看是否有成片偏低的簇；本图**不比两侧色标绝对值**，只看昇腾内部有没有块状异常）：

![昇腾 HBM relmed](card_constitution_20260711_figs/heatmap_relmed_hbm_gbps.svg)

从这张图和 [`COMPARE_ASCEND_MUXI_STABILITY_20260713.md`](COMPARE_ASCEND_MUXI_STABILITY_20260713.md) §4 的结论看，昇腾本批**没有报告整节点级的 HBM 掉速簇**——这是与沐曦（§4）最直接的对比点。

极端 10 卡剖面（按 `sustained_tflops` 挑最慢/最快各 10 卡，看多指标是否一起慢，用来判断「慢是全面慢」还是「单探针噪声」）：

![昇腾极端10卡](constitution_extra_fillgap_20260711_figs/extreme10_small_multiples.svg)

机间体质齐性雷达（各 host 中位相对集群中位，1.0=集群水平；用来看是不是某几个节点整体偏离）：

![昇腾 host 雷达](constitution_extra_fillgap_20260711_figs/radar_host_median_norm.svg)
...
`worker-7`、`worker-14` 两个节点**均为 8/8 卡**HBM 偏慢，约 **1040–1050 GB/s**，相对集群中位（冒烟口径 ~1487 GB/s）明显偏低——**是整节点一起慢，不是某张卡随机噪声**。这个形态本身就是排查方向的线索：如果只是单卡噪声，会去查那张卡的芯片；但「一个节点里全部卡」更该怀疑节点级问题（供电、散热、PCIe 拓扑、驱动版本），运维应该整节点下线复测，而不是逐卡排查。

HBM 相对集群中位偏差热图（host×device；对比 §3 昇腾的同款图，注意本图上是否有连续块状偏低区域）：

![沐曦 HBM relmed](card_constitution_muxi_20260711_figs/heatmap_relmed_hbm_gbps.svg)

极端 10 卡剖面（最慢/最快各 10 卡多指标偏差；用来看 `worker-7`/`worker-14` 的卡是不是在多个指标上一起偏离，还是只有 HBM 单项掉速）：

![沐曦极端10卡](constitution_extra_muxi_20260711_figs/extreme10_small_multiples.svg)

机间体质齐性雷达：

![沐曦 host 雷达](constitution_extra_muxi_20260711_figs/radar_host_median_norm.svg)
...
图证（分 host 箱线 / 全卡分布 / 单卡升序一览，三种视角看同一件事：卡与卡之间是否齐）：

**昇腾**：

![昇腾 launch_sync_p50 分host](card_constitution_20260711_figs/box_by_host_launch_sync_p50_us.svg)
![昇腾 launch_sync_p50 直方图](card_constitution_20260711_figs/hist_launch_sync_p50_us.svg)
![昇腾 launch_sync_p50 升序](card_constitution_20260711_figs/sorted_bar_launch_sync_p50_us.svg)
![昇腾 launch_burst_p50 分host](card_constitution_20260711_figs/box_by_host_launch_burst_p50_us.svg)
![昇腾 launch_burst_p50 直方图](card_constitution_20260711_figs/hist_launch_burst_p50_us.svg)
![昇腾 launch_burst_p50 升序](card_constitution_20260711_figs/sorted_bar_launch_burst_p50_us.svg)
![昇腾 launch 延迟分host三件套](constitution_extra_fillgap_20260711_figs/box_launch_by_host.svg)

**沐曦**：

![沐曦 launch_sync_p50 分host](card_constitution_muxi_20260711_figs/box_by_host_launch_sync_p50_us.svg)
![沐曦 launch_sync_p50 直方图](card_constitution_muxi_20260711_figs/hist_launch_sync_p50_us.svg)
![沐曦 launch_sync_p50 升序](card_constitution_muxi_20260711_figs/sorted_bar_launch_sync_p50_us.svg)
![沐曦 launch_burst_p50 分host](card_constitution_muxi_20260711_figs/box_by_host_launch_burst_p50_us.svg)
![沐曦 launch_burst_p50 直方图](card_constitution_muxi_20260711_figs/hist_launch_burst_p50_us.svg)
![沐曦 launch_burst_p50 升序](card_constitution_muxi_20260711_figs/sorted_bar_launch_burst_p50_us.svg)
![沐曦 launch 延迟分host三件套](constitution_extra_muxi_20260711_figs/box_launch_by_host.svg)
