...
if __name__ == "__main__":
    ...
    x_labels = ["128 GPUs", "256 GPUs", "512 GPUs"]
    
    # 数据格式：[128卡数据, 256卡数据, 512卡数据, 标签名]
    ydata = [
        [235+24, 239+83, 243+67, "HyperSim_autoswitch"],
        [235+278, 239+320, 243+331, "HyperSim"],
        [500000, 600000, 700000, "SimAI"],
        [500000, 600000, 700000, "Astra-sim"],
    ]
    ...
    # 根据数据范围自动设置y轴上限，增加更多空间防止标签重叠
    max_value = max([max(ydata[i][0:len(x_labels)]) for i in range(len(ydata))])
    ax.set_ylim(-MINIMAL, max_value * 1.35)  # 从1.15增加到1.45，提供更多空间
    ...
    plt.savefig('./output/scale_e2e_moe.pdf', bbox_inches='tight', pad_inches=0.1)
